...
author: "Bernardino Bonilla, Lucas Azorín"
date: "11/9/2025"
output:
  html_document:
    theme: cerulean
...
La diabetes es una de las enfermedades crónicas más prevalentes en el mundo actual. Se caracteriza por niveles elevados de glucosa en la sangre debido a una deficiencia en la producción o en la acción de la insulina. En las últimas décadas, su incidencia ha aumentado de manera significativa, convirtiéndose en un problema global de salud pública que afecta a millones de personas en todas las edades y contextos sociales. Diversos factores influyen en el desarrollo de la diabetes, entre ellos los hábitos de vida como la alimentación, la actividad física y el peso corporal. Este trabajo se enfocará en analizar estos factores con el objetivo de conocer la influencia que tiene cada uno en la probabilidad de desarrollar diabetes.
...
ggplot(df, aes(x = family_history_diabetes, fill = diagnosed_diabetes)) +
  geom_bar(position = "dodge") +
  labs(
    title = "Historial Familiar vs Diabetes Diagnosticada",
...
Como se puede observar, el gráfico se divide en dos bloques. El bloque correspondiente al 0 representa a las personas sin familiares con historial de diabetes, mientras que el bloque etiquetado con el 1 incluye a aquellas que sí tienen antecedentes familiares.

Al comparar la proporción de personas diagnosticadas con diabetes en cada grupo, se evidencia claramente que tener antecedentes familiares de diabetes aumenta la probabilidad de desarrollar la enfermedad.
...
tab <- table(df$family_history_diabetes, df$diagnosed_diabetes)
prop.test(tab)
```

Con una prueba de proporciones (prop.test) podremos determinar si existe una correlación entre tener diabetes y tener familiares con diabetes de manera matemática. Hemos elegido hacer esta prueba porque es un modelo que sigue una distribución binomial y, al tener valores binarios, nos ha parecido el más adecuado.

H₀: La probabilidad de tener diabetes es independiente de los antecedentes familiares.

H₁: Tener familiares con diabetes aumenta la probabilidad de padecer diabetes.
...
Como consecuencia, se ve que efectivamente hay una correlación entre tener familia con historial de diabetes y la probabilidad que tiene un sujeto en tener diabetes.
...
ggplot(data, aes(x = diabetes_stage, fill = gender)) +
  ...
  labs(title = "Distribucion de Etapas de Diabetes por Genero",
...
Ahora veremos como afecta que una persona pueda contraer diabetes dependiendo de si ha tenido historial familiar y el tiempo que hace deporte a la semana, a priori lo que nosotros y la mayoría de personas pensamos es que ambas son significativas e influyen mucho en la persona.
```{r}
library(readr)
library(ggplot2)
library(scales)
...
La **hemoglobina glucosilada (HbA1c)** es una medida que mide el porcentaje de hemoglobina en los glóbulos rojos que está unida al azúcar.
...
Nos proporciona un nivel de glucosa en sangre en un momento específico, reflejando el control glucémico después de un periodo de no ingesta.
...
Nos indica cómo el cuerpo gestiona el azúcar después de la ingesta de alimentos y cómo responde el páncreas liberando insulina.

Ahora veremos de estas tres cuál es la mejor para ver cuál tiene una relación mayor y nos sirve para predecir mejor la diabetes.
...
Así a primera vista, nos podemos dar cuenta de que el mejor predictor que tenemos es la **Glucosa Postprandial** o **HbA1c**.  Su coeficiente es alto y estable. 

La glucosa en ayunas y postprandial nos aportan más información útil pero para momentos específicos, por lo cuál serían más unas medidas de monitoreo que de predicción. 
...
Descartaremos la glucosa en ayunas y postprandial; y mantendremos la hemoglobina glicosilada junto con el historial familiar para seguir con una predicción más acertada.
...
Con estas conclusiones llegamos a que tenemos un modelo bueno, pero que podría ser mucho mejor. Esto se debe a que lo que queremos predecir es binario, es decir, la respuesta tiene que ser 1 o 0, esto a través de una regresión lineal es difícil de hacer, tendríamos que usar otro tipo de relación para estimar mejor la probabilidad de tener diabetes. 

## Predicción de Diabetes a través de la predicción de la Hemoglobina Glucosilada(HbA1c)##
...
Con este modelo final, lo primero que podemos decir es que es muy confiable, ya que su R²=0.90. Esto nos dice que con los factores que hemos incluido explicamos el 90.5% de la variabilidad de HbA1c.
